...
char const* sid_y = (char*)(sid+0x1a);

// signed char prev_y = 0;
...
void paint_window(unsigned char x, unsigned char y, unsigned char w, unsigned char h ) {

    for(; y < y+h; y++) {
        unsigned int row = y * screen_width;
        for (; x < x+w; x++) {
            unsigned int col = x;
            *(screen + row + col) = block[bitmap[row + col]];
        }
    }
}
...
void plot_digit(unsigned int x, unsigned int y, char digit) {
...
int main() {
    ...
    *sprite1 = (char)((unsigned int)wall >> 6);
    ...
    char score = 1;
    plot_digit(70, 40, score);
    // paint_window(35, 20, 4, 4);
    ...
    while (ch != 'x' && !game_over) {
        ...
        unsigned int curr_y = *sid_y;

        if (curr_y < wall_bottom-16 && curr_y > wall_top) {
            wall_y = curr_y;
        } else {
            if (curr_y > wall_bottom)
                wall_y = wall_bottom-16;
            if (curr_y < wall_top)
                wall_y = wall_top;
        }
